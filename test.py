...
parser.add_argument('--ensemble_bbb', 
                    action='store_true', 
                    help='Obtain TITAN-BBB testing results for EnsembleBBB')
parser.add_argument('--deepred_bbb', 
                    action='store_true', 
                    help='Obtain TITAN-BBB testing results for DeePred-BBB')
parser.add_argument('--b3clf', 
                    action='store_true', 
                    help='Obtain TITAN-BBB testing results for B3clf')
parser.add_argument('--deep_b3', 
                    action='store_true', 
                    help='Obtain TITAN-BBB testing results for Deep-B3')
args = parser.parse_args()


# Define TITAN-BBB testing dataframe 
test_dataframe = pd.read_csv('data/processed/smiles_test_dataframe.csv')

# Obtain EnsembleBBB TITAN-BBB testing results
ensemble_bbb_results = pd.read_csv('data/benchmark/output/ensemblebbb_prediction_results_titan.csv')

# Obtain DeePred-BBB TITAN-BBB testing results
deepred_bbb_results = pd.read_csv('data/benchmark/output/DeePred-BBB_predictions_titan.csv')

# Obtain B3clf TITAN-BBB testing results
b3clf_results = pd.read_excel('data/benchmark/output/b3clf_predictions_titan.xlsx')

# Obtain Deep-B3 TITAN-BBB testing results 
deep_b3_results = pd.read_csv('data/benchmark/output/deep-b3_predictions_titan.csv')


### Calculate testing results for DeePred-BBB based on the argument ###
if args.deepred_bbb:
    ...
    # Merge DeePred-BBB and TITAN-BBB testing dataframe
    ...
    print(f'{overlapping_smiles_count} compounds from the TITAN-BBB Dataset were removed due to overlap with training dataset')
    print(f'{len(test_dataframe['SMILES'])-len(deepred_bbb_results)} compounds from the TITAN-BBB Dataset were unable to be predicted by the model')
    print(f'{len(test_dataframe['SMILES'])-len(deepred_bbb_results)} compounds were filtered from the TITAN-BBB Dataset in total')
    print('')
...
### Calcuate testing results for EnsembleBBB ###
if args.ensemble_bbb:
    ...
    # Merge EnsembleBBB and TITAN-BBB testing dataframes 
    ensemble_bbb_merged_dataframe = pd.merge(
        left=ensemble_bbb_results, 
        right=test_dataframe, 
        left_on='Smiles', 
        right_on='SMILES', 
        how='left'
    )
    ...
    print(f'{overlapping_smiles_count} compounds were removed from the TITAN-BBB Dataset due to overlaps with the training dataset')
    print(f'{len(test_dataframe['labels'])-len(ensemble_predicted_results)} compounds were filtered from the TITAN-BBB Dataset in total')
    print('')
...
if args.b3clf:
    ...
    # Merge B3clf prediction data and TITAN-BBB dataframe 
    ...
    print(f'{overlapping_smiles_count} compounds were removed from the TITAN-BBB Dataset due to overlaps with the training dataset')
    print(f'{len(test_dataframe['SMILES'])-len(b3clf_results)} compounds were filtered from the TITAN-BBB Dataset in total')
    print('')
...
### Calculate testing results for Deep-B3 ### 
if args.deep_b3:
    ...
    # Merge Deep-B3 and TITAN-BBB testing dataframe
    ...
    print('')
    print(f'{overlapping_smiles_count} compounds were removed from the TITAN-BBB Dataset due to overlap with the training dataset')
    print(f'{len(test_dataframe['SMILES'])-len(deep_b3_results)} compounds were filtered from the TITAN-BBB Dataset in total')
    print('')
