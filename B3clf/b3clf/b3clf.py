...
def b3clf(
    ...
    print(info_df)
    print('This is the end of indo_df')

    smiles_index = []
    import pandas as pd
    index_df = pd.read_csv('b3clf_input_titan.csv', sep='\t')
    for idx in info_df.index:
        smiles_on_index = index_df.loc[index_df.index == idx, 'SMILES']
        smiles_index.append(smiles_on_index.item())

    infos_df = pd.DataFrame(index=smiles_index)
    ...
    # result_df.index = ['row1', 'row2', 'row3']
